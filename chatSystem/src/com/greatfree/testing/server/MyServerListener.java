...
//	public MyServerListener(ServerSocket serverSocket, int threadPoolSize, long keepAliveTime)
	public MyServerListener(ServerSocket serverSocket, ThreadPool pool)
	{
	...
	public void run()
	{
		...
		while (!super.isShutdown())
		{
			try
			{
				...
				serverIO = new MyServerIO(clientSocket, super.getCollaborator(), ServerConfig.CLIENT_PORT);
...
//						ClientPool.SERVER().getPool().send(serverIO.getIP(), ServerConfig.CLIENT_PORT, new NodeKeyNotification(Tools.getKeyOfFreeClient(serverIO.getIP(), ServerConfig.CLIENT_PORT)));
					ClientPool.SERVER().getPool().send(serverIO.getIP(), ServerConfig.CLIENT_PORT, new NodeKeyNotification(serverIO.getRemoteServerKey()));
//					}
				}
