...
import com.greatfree.testing.cluster.coordinator.dn.AnycastReader;
import com.greatfree.testing.cluster.coordinator.dn.AnycastReaderCreator;
import com.greatfree.testing.cluster.coordinator.dn.AnycastReaderDisposer;
import com.greatfree.testing.cluster.coordinator.dn.AnycastReaderSource;
import com.greatfree.testing.cluster.coordinator.dn.AnycastRequestCreator;
import com.greatfree.testing.cluster.coordinator.dn.BroadcastReader;
import com.greatfree.testing.cluster.coordinator.dn.BroadcastReaderCreator;
import com.greatfree.testing.cluster.coordinator.dn.BroadcastReaderDisposer;
import com.greatfree.testing.cluster.coordinator.dn.BroadcastReaderSource;
import com.greatfree.testing.cluster.coordinator.dn.BroadcastRequestCreator;
import com.greatfree.testing.cluster.coordinator.dn.DNServerClientPool;
import com.greatfree.testing.cluster.coordinator.dn.UnicastReader;
import com.greatfree.testing.cluster.coordinator.dn.UnicastReaderCreator;
import com.greatfree.testing.cluster.coordinator.dn.UnicastReaderDisposer;
import com.greatfree.testing.cluster.coordinator.dn.UnicastReaderSource;
import com.greatfree.testing.cluster.coordinator.dn.UnicastRequestCreator;
...
public class CoordinatorMulticastReader
{
	...
	public String broadcastRequest(String request)
	{
		...
		try
		{
			...
			BroadcastReader reader = this.broadcastReaderPool.get(new BroadcastReaderSource(DNServerClientPool.COORDINATE().getPool(), ServerConfig.ROOT_MULTICAST_BRANCH_COUNT, ServerConfig.MULTICAST_BRANCH_COUNT, new BroadcastRequestCreator()));
	...
	public String unicastRequest(String request, String dnKey)
	{
		...
		try
		{
			...
			UnicastReader reader = this.unicastReaderPool.get(new UnicastReaderSource(DNServerClientPool.COORDINATE().getPool(), ServerConfig.ROOT_MULTICAST_BRANCH_COUNT, ServerConfig.MULTICAST_BRANCH_COUNT, new UnicastRequestCreator()));
			// Get the collaborator key of the reader. The key is unique to the reader. 11/29/2014, Bing Li
			String collaboratorKey = reader.resetBroadcast();
	...
	public String anycastRequest(String request)
	{
		...
		try
		{
			...
			AnycastReader reader = this.anycastReaderPool.get(new AnycastReaderSource(DNServerClientPool.COORDINATE().getPool(), ServerConfig.ROOT_MULTICAST_BRANCH_COUNT, ServerConfig.MULTICAST_BRANCH_COUNT, new AnycastRequestCreator()));
			// Get the collaborator key of the reader. The key is unique to the reader. 11/29/2014, Bing Li
			String collaboratorKey = reader.resetBroadcast();
