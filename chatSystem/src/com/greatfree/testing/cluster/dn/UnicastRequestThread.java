...
/*
 * The thread deals with the request and responds to the unicast request initiator that sends the request. Since the request that contains the message must be unicast to the local node's children, the disposal of the request must be controlled by the binder. 11/29/2014, Bing Li
 */

// Created: 11/25/2016, Bing Li
public class UnicastRequestThread extends BoundBroadcastRequestQueue<UnicastRequest, UnicastResponse, MulticastMessageDisposer<UnicastRequest>>
{
	/*
	 * Initialize the thread. The binder is assigned at this point. 11/29/2014, Bing Li
	 */
	public UnicastRequestThread(IPPort ipPort, FreeClientPool pool, int taskSize, String dispatcherKey, MulticastMessageDisposer<UnicastRequest> reqBinder)
	{
		super(ipPort, pool, taskSize, dispatcherKey, reqBinder);
	}

	/*
	 * Deal with the request and respond to the unicast request initiator concurrently. After that, it must notify the binder that its task is accomplished. 11/29/2014, Bing Li
	 */
	public void run()
	{
		// The instance of the received unicast request. 11/29/2014, Bing Li
		UnicastRequest request;
		// The instance of the response to be responded to the unicast request initiator. 11/29/2014, Bing Li
		...
		while (!this.isShutdown())
		{
			...
			while (!this.isEmpty())
			{
				try
				{
					...
					System.out.println("UnicastRequestThread: request = " + request.getRequest());
					...
					response = new UnicastResponse(Constants.UNICAST_RESPONSE, Tools.generateUniqueKey(), request.getCollaboratorKey());
